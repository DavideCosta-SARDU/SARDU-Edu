...
describe('Arduino definitions', () => {
  test('declares the Arduino C++ backend and the initial boards', () => {
    ...
    expect(ARDUINO_HARDWARE_DEFINITIONS.components.get('button')).toMatchObject({
...
describe('generateArduinoSketch', () => {
  ...
  test('generates a Blink sketch from Arduino blocks', () => {
    ...
    expect(source).toContain('// Generated by SARDU-Block - davide@sardu.pro')
  ...
  test('provides an original serial firmware for live mode', () => {
    const firmware = generateSarduBlockLiveFirmware()
    expect(firmware).toContain('SARDU-Block Live firmware - davide@sardu.pro')
    ...
    expect(firmware).toContain('sarduBlockOled->drawRoundRect')
    expect(firmware).toContain('#include <Adafruit_SH110X.h>')
    expect(firmware).toContain("command == 'X'")
    expect(firmware).toContain("command == 'K'")
    expect(firmware).toContain('pinMode(pin, pullup ? INPUT_PULLUP : INPUT);')
    expect(firmware).toContain('Adafruit_SH1106G sarduBlockSh1106(128, 64, &Wire, -1);')
    expect(firmware).toContain('sarduBlockOled->clearDisplay();\n  sarduBlockOled->setTextSize(1);')
    expect(firmware).toContain('sarduBlockSh1106.clearDisplay(); sarduBlockSh1106.setTextSize(1);')
    expect(firmware).toContain('SARDU_BLOCK_OLED_HEART_16[] PROGMEM')
    expect(firmware).toContain('SARDU_BLOCK_OLED_WIFI_32[] PROGMEM')
    expect(firmware).not.toContain('SARDU_BLOCK_OLED_ARDUINO')
    expect(firmware).toContain('SARDU-BLOCK-LIVE 10')
  })

  test('configures active-low and active-high buttons from their Boolean blocks', () => {
    const lowBlocks = {
      program: block('program', 'sarduBoard_program', {topLevel: true,
        inputs: {SUBSTACK: {block: 'configure'}, SUBSTACK2: {block: 'condition'}}}),
      configure: block('configure', 'sarduSensors_configureButton', {fields: {PIN: {value: '2'}}}),
      condition: block('condition', 'control_if', {inputs: {CONDITION: {block: 'pressed'}}}),
      pressed: block('pressed', 'sarduSensors_buttonPressedLow', {fields: {PIN: {value: '2'}}}),
    }
    const highBlocks = {
      program: block('program', 'sarduBoard_program', {topLevel: true,
        inputs: {SUBSTACK: {block: 'configure'}, SUBSTACK2: {block: 'condition'}}}),
      configure: block('configure', 'sarduSensors_configureButton', {fields: {PIN: {value: '3'}}}),
      condition: block('condition', 'control_if', {inputs: {CONDITION: {block: 'pressed'}}}),
      pressed: block('pressed', 'sarduSensors_buttonPressedHigh', {fields: {PIN: {value: '3'}}}),
    }

    const lowSource = generateArduinoSketch({boardId: 'arduino-uno', targets: [{blocks: lowBlocks}]})
    const highSource = generateArduinoSketch({boardId: 'arduino-uno', targets: [{blocks: highBlocks}]})
    expect(lowSource).toContain('pinMode(2, INPUT_PULLUP);')
    expect(lowSource).toContain('if ((digitalRead(2) == LOW))')
    expect(highSource).toContain('pinMode(3, INPUT);')
    expect(highSource).toContain('if ((digitalRead(3) == HIGH))')
  })

  test('rejects incompatible button modes on the same pin', () => {
    const blocks = {
      program: block('program', 'sarduBoard_program', {topLevel: true,
        inputs: {SUBSTACK: {block: 'configure'}, SUBSTACK2: {block: 'first'}}}),
      configure: block('configure', 'sarduSensors_configureButton', {fields: {PIN: {value: '2'}}}),
      first: block('first', 'control_if', {next: 'second', inputs: {CONDITION: {block: 'low'}}}),
      low: block('low', 'sarduSensors_buttonPressedLow', {fields: {PIN: {value: '2'}}}),
      second: block('second', 'control_if', {inputs: {CONDITION: {block: 'high'}}}),
      high: block('high', 'sarduSensors_buttonPressedHigh', {fields: {PIN: {value: '2'}}}),
    }
    expect(() => generateArduinoSketch({boardId: 'arduino-uno', targets: [{blocks}]}))
      .toThrow('Button pin 2 cannot use both INPUT_PULLUP and INPUT')
  })

  test('requires the button configuration block before reading a pin', () => {
    const blocks = {
      program: block('program', 'sarduBoard_program', {topLevel: true, inputs: {SUBSTACK2: {block: 'condition'}}}),
      condition: block('condition', 'control_if', {inputs: {CONDITION: {block: 'pressed'}}}),
      pressed: block('pressed', 'sarduSensors_buttonPressedLow', {fields: {PIN: {value: '2'}}}),
    }
    expect(() => generateArduinoSketch({boardId: 'arduino-uno', targets: [{blocks}]}))
      .toThrow('Configure the button on pin 2 before reading it')
  })
  ...
  test('generates 1602 I2C display operations with constrained zero-based coordinates', () => {
    ...
    expect(source).toContain('sarduBlockDisplay->begin(16, 2, Wire);')
    expect(source).toContain('sarduBlockDisplay->setCursor(constrain(20, 0, 15), constrain(4, 0, sarduBlockDisplayRows - 1));')
    expect(source).toContain('sarduBlockDisplay->print("Valore");')
    expect(source).toContain('sarduBlockDisplay->setBacklight(0);')
    expect(source).toContain('sarduBlockDisplay->cursor();')
    expect(source).toContain('sarduBlockDisplay->noBlink();')
    expect(source).toContain('sarduBlockDisplay->clear();')
  })
  ...
  test('generates custom display pins only for ESP32 boards', () => {
    ...
    expect(source).toContain('new LiquidCrystal_PCF8574(0x27);')
    expect(source).toContain('sarduBlockDisplay->begin(16, 4, Wire);')
    expect(() => generateArduinoSketch({boardId: 'arduino-uno', targets: [{blocks}]})).toThrow(
  ...
  test('generates OLED support and bitmap data only for connected OLED blocks', () => {
    ...
    expect(source).not.toContain('delete sarduBlockOled')
    expect(source).not.toContain('sarduBlockOled->')
    expect(source).toContain('oled.setCursor(0, 0);')
    expect(source).toContain('oled.println("Ciao");')
    expect(source).toContain('SARDU_BLOCK_OLED_HEART_16[] PROGMEM')
    expect(source).not.toContain('SARDU_BLOCK_OLED_STAR_16[] PROGMEM')
    expect(source).toContain('sarduBlockDrawOledImage(SARDU_BLOCK_OLED_HEART_16, 16, 1);')
    expect(source).toContain('SARDU_BLOCK_OLED_WIFI_32[] PROGMEM')
    expect(source).toContain('sarduBlockDrawOledImage(SARDU_BLOCK_OLED_WIFI_32, 32, 2);')
  ...
  test('localizes the generated sketch header', () => {
    ...
    expect(source).toContain('// Generato da SARDU-Block - davide@sardu.pro')

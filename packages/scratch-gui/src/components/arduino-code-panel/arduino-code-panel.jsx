...
import {ARDUINO_BOARDS, generateArduinoSketch, generateSarduBlockLiveFirmware} from '@sardu-block/hardware';
...
import SarduBlockSerialTransport from '../../lib/sardu-serial-transport';
...
const ArduinoCodePanel = ({
    ...
    if (!transportRef.current) {
        transportRef.current = new SarduBlockSerialTransport(undefined, diagnostic => {
            const entry = {...diagnostic, ...(selectedPortRef.current ? {port: selectedPortRef.current} : {})};
            console.info('SARDU-Block Live diagnostic', entry);
            onStatusChange({
                ...
                message: intl.formatMessage({
                    ...
                    description: 'Detailed status for a SARDU-Block Live serial operation'
                }, {
                    stage: entry.stage,
    ...
    const [nanoProcessor, setNanoProcessor] = useState('auto');
    ...
    const updateSource = useCallback(() => {
        const selection = vm.getSarduBlockProjectData()?.hardwareSelection || null;
        if (!selection) {
            ...
            const source = generateArduinoSketch({
                ...
                messages: {
                    ...
                    pn532NotFound: intl.formatMessage({
                        id: 'gui.sarduBlock.pn532NotFound',
                    ...
                    generatedBy: intl.formatMessage({
                        id: 'gui.sarduBlock.generatedBy',
                        defaultMessage: 'Generated by',
                        description: 'Generated Arduino code header label'
                    }),
                    board: intl.formatMessage({
                        id: 'gui.sarduBlock.generatedBoard',
                        defaultMessage: 'Board',
                        description: 'Generated Arduino code board label'
                    })
                },
    ...
    const refreshDesktop = useCallback(async (showLoading = true) => {
        ...
        if (!hardware) {
            ...
            vm.setSarduBlockSelectedPort(null);
        ...
        try {
            ...
            const selectedBoardId = vm.getSarduBlockProjectData()?.hardwareSelection?.boardId;
            ...
            const confirmedPort = vm.runtime.sarduEduHardwarePort;
            const previousPort = selectedPortRef.current;
            const selectedPort = confirmedPort || getPreselectedPort(ports, previousPort);
            selectedPortRef.current = selectedPort;
            vm.setSarduBlockSelectedPort(selectedPort || null);
            setDesktopState(current => ({
                ...current,
                available: true,
                checked: true,
    ...
    useEffect(() => {
        const events = ['PROJECT_CHANGED', 'targetsUpdate', 'workspaceUpdate', 'SARDU_BLOCK_HARDWARE_CHANGED'];
    ...
    useEffect(() => {
        const handleConnectionChange = async () => {
            ...
            if (!connectedPort) {
                vm.setSarduBlockLiveTransport(null);
                try {
            ...
            try {
                ...
                vm.setSarduBlockLiveTransport(transportRef.current);
        ...
        vm.on('SARDU_BLOCK_HARDWARE_CHANGED', handleConnectionChange);
        void handleConnectionChange();
        return () => vm.removeListener('SARDU_BLOCK_HARDWARE_CHANGED', handleConnectionChange);
    ...
    useEffect(() => {
        ...
        vm.on('SARDU_BLOCK_CONNECT_REQUESTED', handleConnectRequest);
        return () => vm.removeListener('SARDU_BLOCK_CONNECT_REQUESTED', handleConnectRequest);
    }, [refreshDesktop, vm]);
    useEffect(() => {
    ...
    const handleModeChange = async event => {
        ...
        vm.setSarduBlockHardwareSelection({...snapshot.selection, mode});
        setLiveState({status: 'disconnected', error: null});
                            ...
                            <p>
                                {desktopState.ports.length === 0 ? (
                                    ...
                                    <FormattedMessage
                                        ...
                                        defaultMessage="Multiple possible boards were found. Select the port you want to use; SARDU-Block will not choose automatically."
                            ...
                            {desktopState.ports.length ? (
                                ...
                                <select
                                    ...
                                    onChange={event => {
                                        ...
                                        vm.setSarduBlockSelectedPort(event.target.value || null);
                            ...
                            {desktopState.ports.length && snapshot.selection.boardId === 'arduino-nano' ? (
                                <label className={styles.connectionPromptPort}>
                                    <FormattedMessage
                                        ...
                                        defaultMessage="Nano bootloader"
                                        description="Nano bootloader selector label"
                                    />
                                    <select value={nanoProcessor} onChange={event => setNanoProcessor(event.target.value)}>
                                        <option value="auto">
                                            {intl.formatMessage({id: 'gui.sardu.nanoBootloaderAutomatic'})}
                                        </option>
                                        <option value="new">
                                            {intl.formatMessage({id: 'gui.sardu.nanoBootloaderNew'})}
                                        </option>
                                        <option value="old">
                                            {intl.formatMessage({id: 'gui.sardu.nanoBootloaderOld'})}
                                        </option>
                                    </select>
                                </label>
                            ) : null}
                            <div className={styles.connectionPromptActions}>
                                <button
                                    ...
                                    onClick={() => {
                                        ...
                                        onStatusChange({
                                            kind: 'neutral',
                                            message: intl.formatMessage({
                                                id: 'gui.sarduBlock.status.connectionCancelled',
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        ...
                                        connectionPromptRequestedRef.current = false;
                                        vm.setSarduBlockHardwarePort(desktopState.selectedPort);
                                        setConnectionPromptOpen(false);
                                        onStatusChange({
                                            kind: 'success',
                                            message: intl.formatMessage({
                                                id: 'gui.sarduBlock.status.connectionConfirmed',
                    ...
                    <React.Fragment>
                        ...
                        >
                            <FormattedMessage
                                ...
                                description="Button that compiles and uploads the SARDU-Block Live firmware"
                            />
                        </button>
                    </React.Fragment>
                )}
                    ...
                    <ol>
                        ...
                        <li className={
                            ...
                            <FormattedMessage
                                ...
                                defaultMessage="Install the SARDU-Block Live firmware"
            ...
            <div className={styles.liveStatus}>
                {!desktopState.checked || desktopState.status === 'loading' ? (
                    ...
                    <FormattedMessage
                        ...
                        defaultMessage="Compilation and upload require the SARDU-Block desktop application."

...
describe('resizable flyout', () => {
  it('updates its width and creates a horizontal scrollbar', () => {
    assert(container, 'Expected container from beforeEach')
    workspace = Blockly.inject(container, {
      toolbox: {
        kind: 'flyoutToolbox',
        contents: [{ kind: 'block', type: 'test_value_block' }],
      },
      plugins: {
        flyoutsVerticalToolbox: CheckableContinuousFlyout,
      },
    ...
    const flyout = workspace.getFlyout() as CheckableContinuousFlyout
    const horizontalScrollbar = (flyout as unknown as { horizontalScrollbar: Blockly.Scrollbar }).horizontalScrollbar
    const setOrigin = vi.spyOn(horizontalScrollbar, 'setOrigin')
    flyout.setWidth(360)

    expect(flyout.getWidth()).toBe(360)
    expect(setOrigin).toHaveBeenCalled()
    expect(container.querySelector('.blocklyFlyoutScrollbar.blocklyScrollbarHorizontal')).not.toBeNull()

    flyout.setWidth(40)
    const initialScrollX = flyout.getWorkspace().scrollX
    const flyoutMetrics = flyout as unknown as { setMetrics_: (ratio: { x: number; y: number }) => void }
    flyoutMetrics.setMetrics_({ x: 1, y: 0 })

    expect(flyout.getWorkspace().scrollX).not.toBe(initialScrollX)

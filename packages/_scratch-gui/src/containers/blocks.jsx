...
class Blocks extends React.Component {
    ...
    setupToolboxResizeHandle () {
        ...
        if (!toolboxElement || !flyoutBackgroundElement || typeof ResizeObserver === 'undefined') return;
        ...
        const updateHandlePosition = () => {
            ...
            resizeHandle.style.left = `${Math.max(0, flyoutRight - blocksLeft - 12)}px`;
        };
        this.toolboxResizePointerDown = event => {
            this.toolboxResizeState = {
                startFlyoutWidth: flyout.getWidth(),
                startX: event.clientX,
                maxWidth: Math.max(160, (this.blocks.getBoundingClientRect().width * 0.6) -
                    toolboxElement.getBoundingClientRect().width)
            };
        ...
        this.toolboxResizePointerMove = event => {
            ...
            const flyoutWidth = Math.max(160, Math.min(this.toolboxResizeState.maxWidth,
                this.toolboxResizeState.startFlyoutWidth + delta));
            flyout.width_ = flyoutWidth;
            flyout.position();
